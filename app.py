...
def analytics():
    ...
    if organization_id:
        org_depts = Department.query.filter_by(organization_id=organization_id, is_active=True).all()
        departments = [d.name for d in org_depts]
    elif school_id:
        departments = get_departments_for_school(school_id)
    else:
    ...
    department_labels = []
    department_data = []
    unique_depts = list(set(s.department for s in all_staff if s.department))
...
# ============== MIGRATION ROUTE (REMOVE AFTER USE) ==============

@app.route('/migrate-departments')
def migrate_departments():
    """Migration route to add department tables and is_school column"""
    try:
        from sqlalchemy import text
        
        migrations = [
            # Add is_school column to organizations
            "ALTER TABLE organizations ADD COLUMN IF NOT EXISTS is_school BOOLEAN DEFAULT TRUE",
        ...
        for org in orgs:
            existing = Department.query.filter_by(organization_id=org.id).first()
            if not existing:
                is_school = getattr(org, 'is_school', True)
                if is_school is None:
                    is_school = True
                ...
                if is_school:
                    ...
                    defaults = ['Operations', 'Finance', 'Human Resources', 'IT', 'Marketing', 'Administration', 'Management']
        ...
        <body style="font-family: Arial, sans-serif; padding: 40px; text-align: center;">
            <h1 style="color: green;">✓ Migration Complete!</h1>
            <p>Departments table created and default departments added to all organizations.</p>
            <br>
            <a href="/settings" style="background: #667eea; color: white; padding: 15px 30px; text-decoration: none; border-radius: 5px; font-size: 18px;">Go to Settings</a>
            <br><br>
            <p style="color: red;"><strong>Important:</strong> Remove the /migrate-departments route from app.py after confirming everything works!</p>
        </body>
        </html>
        '''
    except Exception as e:
...
def init_db():
    try:
        ...
        try:
            db.session.execute(db.text('ALTER TABLE organizations ADD COLUMN is_school BOOLEAN DEFAULT TRUE'))
            db.session.commit()
        except:
            db.session.rollback()
